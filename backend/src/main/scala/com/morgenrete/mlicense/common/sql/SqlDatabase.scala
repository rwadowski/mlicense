package com.morgenrete.mlicense.common.sql

import java.net.URI
import java.time.{OffsetDateTime, ZoneOffset}

import com.morgenrete.mlicense.config.DatabaseConfig
...
object SqlDatabase extends StrictLogging {
  ...
  def createPostgresFromConfig(config: DatabaseConfig) = {
    val db = Database.forConfig("mlicense.db.postgres", config.rootConfig)
  ...
  private def createEmbedded(config: DatabaseConfig): SqlDatabase = {
    val db = Database.forConfig("mlicense.db.h2")
