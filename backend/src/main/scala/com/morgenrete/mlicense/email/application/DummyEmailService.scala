...
import com.morgenrete.mlicense.email.application.SmtpEmailSender.EmailDescription
...
class DummyEmailService extends EmailService with StrictLogging {
  ...
  override def scheduleEmail(address: String, emailData: EmailContentWithSubject): Future[Unit] = {
